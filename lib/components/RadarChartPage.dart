  ...
  void drawRadarChartBackground() {
    ...
    widget.maps.keys.forEach((key) {
      painterSmall.text = new TextSpan(
          text: key, style: new TextStyle(fontSize: 15.0, color: Colors.black));
      painterSmall.layout();
    ...
    maxWidth = widget.radarChartRadius +
        maxTextWidth * 2 +
        spaceBetweenChartAndText * 2;
    maxHeight = widget.radarChartRadius +
        painterSmall.size.height * 2 +
        spaceBetweenChartAndText * 2;
    ...
    for (int i = 0; i < widget.maps.length; i++) {
      ...
      for (int m = 0; m < 5; m++) {
        ...
        if (4 == m) {
          ...
          canvas.drawLine(
              new Offset(-w, 0), new Offset(0, linesRadius), linePaint);
        }
        canvas.restore();
      }
      canvas.rotate(angle);
    ...
    for (int i = 0; i < widget.maps.length; i++) {
      ...
      painterSmall.text = new TextSpan(
          text: widget.maps.keys.elementAt(i),
          style: new TextStyle(fontSize: 15.0, color: Colors.black));
      painterSmall.layout();

      if (accumulateAngle >= (1.875 * pi) || accumulateAngle < pi / 8) {
        //0
        painterSmall.paint(
            canvas,
            new Offset(xPoint + spaceBetweenChartAndText,
                yPoint - painterSmall.size.height / 2));
      } else if (accumulateAngle >= pi / 8 && accumulateAngle < pi * 0.375) {
        painterSmall.paint(
            canvas,
            new Offset(xPoint + spaceBetweenChartAndText,
                yPoint - painterSmall.size.height / 2));
      } else if (accumulateAngle >= pi * 0.375 &&
          accumulateAngle < pi * 0.625) {
        //90
        painterSmall.paint(
            canvas,
            new Offset(xPoint - painterSmall.size.width / 2,
                yPoint + spaceBetweenChartAndText));
      } else if (accumulateAngle >= pi * 0.625 &&
          accumulateAngle < pi * 0.875) {
        painterSmall.paint(
            canvas,
            new Offset(
                xPoint - painterSmall.size.width - spaceBetweenChartAndText,
                yPoint - painterSmall.size.height / 2));
      } else if (accumulateAngle >= pi * 0.875 &&
          accumulateAngle < pi * 1.125) {
        //180
        painterSmall.paint(
            canvas,
            new Offset(
                xPoint - painterSmall.size.width - spaceBetweenChartAndText,
                yPoint - painterSmall.size.height / 2));
      } else if (accumulateAngle >= pi * 1.125 &&
          accumulateAngle < pi * 1.375) {
        painterSmall.paint(
            canvas,
            new Offset(
                xPoint - painterSmall.size.width - spaceBetweenChartAndText,
                yPoint - painterSmall.size.height / 2));
      } else if (accumulateAngle >= pi * 1.375 &&
          accumulateAngle < pi * 1.625) {
        //270
        painterSmall.paint(
            canvas,
            new Offset(xPoint - painterSmall.size.width / 2,
                yPoint - spaceBetweenChartAndText - painterSmall.size.height));
      } else {
        painterSmall.paint(
            canvas,
            new Offset(xPoint + spaceBetweenChartAndText,
