...
class AnimationState extends State<AnimationWidgetPage>
  ...
  void initState() {
    ...
    imagesPaths.add("assets/images/chaonan3.jpeg");
    imagesPaths.add("assets/images/chaonan4.jpeg");
    imagesPaths.add("assets/images/chaonan2.jpg");
    imagesPaths.add("assets/images/chaonan1.jpeg");
  }
   ...
   */
  Row creatRow(
      double imgHeight, double imgWidth, double maginSize, String imgPath) {
    return new Row(
  ...
  Widget build(BuildContext context) {
    return new Scaffold(
      appBar: new AppBar(
        title: new Text("模仿京东品质潮男页面的动画效果"),
      ),
      body: new Center(
        ...
        onHorizontalDragUpdate: (getureDragUpdateCallback) {
          if ((preXPosition - getureDragUpdateCallback.globalPosition.dx) >
                  50 &&
              preXPosition != 0.0) {
            print("动画启动");
            preXPosition = 0.0;
            directionLeft = true;
            controller.forward();
          } else if ((preXPosition -
                      getureDragUpdateCallback.globalPosition.dx) <
                  -50 &&
              preXPosition != 0.0) {
            print("动画启动");
            preXPosition = 0.0;
        ...
        child: new Stack(
          children: <Widget>[
            ...
            creatRow(
                imageHeight * (1.9 + 0.3 * scales),
                imageWidth * (1.9 + 0.3 * scales),
                30.0 + scales * magins,
                imagesPaths[1]),
            creatRow(
                imageHeight * (2.2 + 0.3 * scales),
                imageWidth * (2.2 + 0.3 * scales),
                magins + 30 + scales * magins,
                imagesPaths[2]),
            creatRow(
                imageHeight * 2.5,
                imageWidth * 2.5,
                magins * 2 + 30 + scales * (imageWidth * 2.5 + 40),
                imagesPaths[3]),
          ],
        ),
      )),
