...
class BubblesPageState extends State<BubblesPage> {
          ...
          setState(() {
            ...
            for (int i = 0; i < _bubbles.length; i++) {
              ...
              if (bubbleBin.yPosition <= _disappearHeight + _fixedHeight) {
                int alpha = (bubbleBin.yPosition /
                        (_disappearHeight + _fixedHeight) *
                        0xFF)
                    .ceil();
            ...
            if (xPositionRandom > xBlankSpace &&
                xPositionRandom < 1.0 - xBlankSpace &&
                yPositionRandom > 0.2 &&
                radiusRandom > 0.2 &&
                speedRandom > 0.2 &&
                _bubbles.length < 10) {
              _disappearHeight = screenHeight -
                  (_fixedHeight +
                      _fixedHeight *
                          _getPercentageOfDisappearHeight(yPositionRandom));
  ...
  double _getPercentageOfDisappearHeight(double doubleRandom) {
    ...
    if (doubleRandom > 0.3) {
      percentageOfDisappearHeight =
          _getPercentageOfDisappearHeight(doubleRandom / 2);
    } else {
