...
class IosTimePackerPage extends StatefulWidget {
  @override
  State<StatefulWidget> createState() {
    return new IosTimePackerState();
  }
}

class IosTimePackerState extends State<IosTimePackerPage>
          ...
          child: new Container(
            ...
            child: new GestureDetector(
                ...
                onVerticalDragEnd: (getureDragEndCallback) {
                  ...
                  if (DateTime.now().millisecondsSinceEpoch - downTime < 300 &&
                      lastOffset < 15 &&
                      distance == 0.0) {
                    return;
                  }
                  double controlOffset =
                      paintCanvas.getOffset(distance + lastOffset.abs());
                  ...
                  if (velo.pixelsPerSecond.dy == 0.0) {
                    frictionSimulation = new FrictionSimulation.through(
                        0.0, distance + 0.1, 20.0, 0.0);
                    scrollOffset = frictionSimulation.timeAtX(distance);
                  } else {
                    frictionSimulation = new FrictionSimulation.through(
                        0.0, distance, 20.0, 0.0);
                    scrollOffset =
                        frictionSimulation.timeAtX(distance - controlOffset);
                  }

                  animation = new Tween(begin: 0.0, end: scrollOffset)
                      .animate(controller);
                  voidCallback = () {
                    ...
                    if (offset < 0) {
                      offset =
                          -(frictionSimulation.x(animation.value)) + lastOffset;
                    } else {
                      offset =
                          frictionSimulation.x(animation.value) + lastOffset;
                    }
                  ...
                  statusListener = (animationStatus) {
                    if (animationStatus == AnimationStatus.completed) {
                      ...
                      animation.removeStatusListener(statusListener);
                      controller.reset();
                      Scaffold.of(contextx).showSnackBar(new SnackBar(
                        content: new Text(hours[3]),
...
class TextPaintCanvas extends CustomPainter {
  ...
  double getRadian(double itemOffset, double controlHeight, double maginsHeight,
      double smallTextHright) {
    double radian;
  ...
  void drawText(Canvas canvas, TextPainter painterSmall, double itemOffset,
      String texts) {
    canvas.save();
    canvas.translate(0.0, itemOffset);
    painterSmall.text = new TextSpan(
        text: texts, style: new TextStyle(fontSize: 18.0, color: Colors.black));
    painterSmall.layout();
    ...
    try {
      canvas.transform(Matrix4.rotationX(
              getRadian(itemOffset, 130.0, maginsHeight, smallTextHright))
          .storage);
    } catch (e) {}
  ...
  void paint(Canvas canvas, Size size) {
    ...
    //第一条Item,初始化不可见
    drawText(canvas, painterSmall, -maginsHeight - smallTextHright + offset,
        hours[0]);
    //第二条Item
    ...
    drawText(canvas, painterSmall, maginsHeight + smallTextHright + offset,
        hours[2]);
    //最中间的Item
    drawText(canvas, painterSmall,
        maginsHeight * 2 + smallTextHright * 2 + offset, hours[3]);
    //第五条Item
    drawText(canvas, painterSmall,
        maginsHeight * 3 + smallTextHright * 3 + offset, hours[4]);
    //第六条Item
    drawText(canvas, painterSmall,
        maginsHeight * 4 + smallTextHright * 4 + offset, hours[5]);
    //第七条Item
    drawText(canvas, painterSmall,
        maginsHeight * 5 + smallTextHright * 5 + offset, hours[6]);

    Rect rect =
        Rect.fromLTRB(0.0, 0.0, 80.0, maginsHeight * 2 + smallTextHright * 2);
